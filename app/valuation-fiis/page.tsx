...
import { calcularTIRFII, formatarNumero } from "@/lib/calculators/tir-utils"
import {
...
export default function ValuationFIIs() {
  ...
  const [taxaDesconto, setTaxaDesconto] = useState("14")
  ...
  const calcular = () => {
    ...
    let dividendoAtual = dividendoAnualNum
    ...
    const dividendoProximoAno = dividendosProjetados[2] * (1 + taxaCrescimentoPerpetuaNum / 100)
    const valorSaida = dividendoProximoAno / (taxaDescontoNum / 100 - taxaCrescimentoPerpetuaNum / 100)
    ...
    setTir(calcularTIRFII(precoAtualNum, dividendosProjetados, taxaCrescimentoPerpetuaNum, taxaDescontoNum))

    // Gerar matriz de sensibilidade
    const calculaValorJustoFn = (taxaCrescimento: number, taxaDesconto: number) => {
      return calcularValorJusto(dividendoAnualNum, taxaCrescimento, taxaCrescimentoPerpetuaNum, taxaDesconto)
    }
  ...
  const calcularValorJusto = (
    ...
    let vpDividendos = 0
    let dividendoAno3 = dividendoAtual

    // Cálculo do VP dos dividendos futuros (anos 1-3)
    for (let ano = 1; ano <= 3; ano++) {
      dividendoAno3 =
        ano === 1 ? dividendoAtual * (1 + taxaCrescimentoDecimal) : dividendoAno3 * (1 + taxaCrescimentoDecimal)
      vpDividendos += dividendoAno3 / Math.pow(1 + taxaDescontoDecimal, ano)
    }

    // Cálculo do valor terminal
    const dividendoAno4 = dividendoAno3 * (1 + taxaPerpetuaDecimal)

    // Verificar se a taxa de desconto é maior que a taxa de crescimento perpétua
    if (taxaDescontoDecimal <= taxaPerpetuaDecimal) {
      return vpDividendos + (dividendoAno4 * 20) / Math.pow(1 + taxaDescontoDecimal, 3) // Usar múltiplo de 20x se a fórmula não for aplicável
    }

    const valorPerpetuo = dividendoAno4 / (taxaDescontoDecimal - taxaPerpetuaDecimal)
    const vpValorPerpetuo = valorPerpetuo / Math.pow(1 + taxaDescontoDecimal, 3)

    // Valor justo total
    return vpDividendos + vpValorPerpetuo
  }
  ...
  return (
    <>
      <Head>
        <title>Valuation de Fundos Imobiliários | Calculadora de Valor Justo de FIIs</title>
        <meta
          name="description"
          content="Determine o valor justo de fundos imobiliários com base nos dividendos projetados. Ferramenta para investidores da bolsa brasileira avaliarem FIIs."
        />
        <meta
          name="keywords"
          content="valuation de fundos imobiliários, valor justo de FIIs, análise de FIIs, dividendos, fundos imobiliários"
        />
        <link rel="canonical" href="https://valorjusto.com.br/valuation-fiis" />
        <meta property="og:title" content="Valuation de Fundos Imobiliários | Calculadora de Valor Justo de FIIs" />
        <meta
          property="og:description"
          content="Determine o valor justo de fundos imobiliários com base nos dividendos projetados."
        />
        <meta property="og:url" content="https://valorjusto.com.br/valuation-fiis" />
      ...
      <div className="min-h-screen bg-[#f5f9f7]">
        ...
        <div className="container mx-auto px-4 py-6">
          ...
          <h1 className="text-xl font-bold mb-2">Valor Justo de FIIs</h1>
            ...
            <div className="md:col-span-5">
              <div className="bg-white rounded-lg shadow-sm p-6">
                ...
                <p className="text-xs text-gray-500 mb-4">
                  Preencha os campos abaixo para calcular o valor justo do FII
                </p>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="tooltip ml-2">
                      ...
                      <span className="tooltip-text">Código do FII na B3</span>
                    </div>
                  </div>
                ...
                <div className="mb-4">
                  <label htmlFor="nome-fundo" className="block text-sm font-medium mb-1">
                    Nome do Fundo
                  </label>
                  <div className="flex items-center">
                    <input
                      id="nome-fundo"
                      type="text"
                      placeholder="Kinea Renda Imobiliária"
                      value={nomeFundo}
                    ...
                    <div className="tooltip ml-2">
                      <InfoIcon className="h-4 w-4 text-gray-400" aria-hidden="true" />
                      <span className="tooltip-text">Nome do fundo (opcional)</span>
                    </div>
                  </div>
                </div>

                <div className="mb-4">
                  <label htmlFor="dividendo-anual" className="block text-sm font-medium mb-1">
                    Dividendo pago no último ano (R$)
                  </label>
                  <div className="flex items-center">
                    <input
                      id="dividendo-anual"
                      ...
                      className={`input-field ${errors.dividendoAnual ? "border-red-500" : ""}`}
                    />
                    <div className="tooltip ml-2">
                      <InfoIcon className="h-4 w-4 text-gray-400" aria-hidden="true" />
                      <span className="tooltip-text">
                        Dividendo total pago nos últimos 12 meses (você pode encontrar esses dados no Status Invest,
                        Investidor10 ou Funds Explorer)
                      </span>
                    </div>
                  </div>
                  {errors.dividendoAnual && <p className="text-red-500 text-xs mt-1">{errors.dividendoAnual}</p>}
                </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="tooltip ml-2">
                      <InfoIcon className="h-4 w-4 text-gray-400" aria-hidden="true" />
                      <span className="tooltip-text">Preço atual da cota</span>
                    </div>
                  </div>
                ...
                <div className="mb-4">
                  <label htmlFor="taxa-crescimento" className="block text-sm font-medium mb-1">
                    Taxa de Crescimento Anual (3 anos) (%)
                  </label>
                  <div className="flex items-center">
                    <input
                      id="taxa-crescimento"
                    ...
                    <div className="tooltip ml-2">
                      <InfoIcon className="h-4 w-4 text-gray-400" aria-hidden="true" />
                      <span className="tooltip-text">
                        Taxa de crescimento anual esperada para os dividendos nos próximos 3 anos
                      </span>
                    </div>
                  </div>
                ...
                {!calculado ? (
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="bg-gray-100 p-4 rounded mb-4">
                      <p className="text-sm">
                        <span className="font-medium">Segundo suas premissas, o FII está: </span>
                        {potencialValorizacao! >= 0 ? (
                          <span className="text-[#0d9f6f] font-bold">Subavaliado</span>
                        ) : (
                          <span className="text-red-500 font-bold">Sobreavaliado</span>
                        )}
                      </p>
                    ...
                    {/* Seção de TIR */}
                    <div>
                      <h3 className="font-medium mb-2">Qual o retorno anual esperado para esse FII?</h3>
                      <p className="text-sm text-gray-500 mb-3">
                        Considerando um investimento de 3 anos, pelas suas premissas, você obterá a seguinte taxa
                        interna de retorno:
                      </p>
                    ...
                    <TIRSection tir={tir} fluxos={fluxosCaixa} ticker={ticker} anos={3} tipoInvestimento="fii" />

                    {/* Análise de Sensibilidade */}
                    <div className="pt-4">
                      <h3 className="text-lg font-medium mb-2">Análise de Sensibilidade</h3>
                      <p className="text-xs text-gray-500 mb-4">
                        A matriz abaixo demonstra o valor justo do FII variando a taxa de crescimento e a taxa de
                        desconto
                      </p>
